...
class Vits(BaseTTS):
    ...
    def init_multispeaker(self, config: Coqpit):
        ...
        print('------------------------------num_speakers:',self.num_speakers)
        self.audio_transform = None
        print('---------------------if self.speaker_manager:',self.speaker_manager)
        if self.speaker_manager:
            self.num_speakers = self.speaker_manager.num_speakers
            print('----#########--------self.num_speakers:',self.num_speakers)
        ...
        if self.args.use_speaker_encoder_as_loss:
            # print('self.spaker_manager.speaker_encoder:',self.speaker_manager.speaker_encoder)
            # print(self.args.speaker_encoder_model_path)
            # print(self.args.speaker_encoder_config_path)
    ...
    def forward(  # pylint: disable=dangerous-default-value
        ...
        if self.args.use_speaker_encoder_as_loss and self.speaker_manager.speaker_encoder is not None:
            ...
            pred_embs = self.speaker_manager.speaker_encoder.forward(wavs_batch.cuda(), l2_norm=True)
    ...
    def voice_conversion(self, y, y_lengths, speaker_cond_src, speaker_cond_tgt):
        ...
        print('y.transpose(1,2).shape',y.transpose(1,2).shape)
        print('-----------------------------g_src.shape:',g_src.shape)
        print('-----------------------------g_tgt.shape:',g_tgt.shape)
