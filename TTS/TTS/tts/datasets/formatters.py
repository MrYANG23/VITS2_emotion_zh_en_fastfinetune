...
def yh_baker(root_path:str,meta_file:str,**kwargs) -> List[List[str]]:
    import re
    ...
    with open(txt_file,"r",encoding="utf-8") as f:
        all_lables=f.readlines()


        for line in all_lables[::2]:
            name,text=line.strip().split('\t')
            text=re.sub("[#a-zA-Z0-9]+","",text)
            wav_file=os.path.join(root_path,'Wave',name+'.wav')
            items.append({"text":text,"audio_file":wav_file,"speaker_name":spaker_name})
    # print(items)
    # print(len(items))
    # exit()
    return items

def aishell3(root_path:str,meta_file:str,**kwargs) -> List[List[str]]:
    txt_file = os.path.join(root_path, meta_file)
    items = []
    import re
    with open(txt_file,"r",encoding='utf-8') as f:
        all_labels=f.readlines()
        for per_line in tqdm(all_labels[5:]):
            name,pingyins,text=per_line.strip().split('|')
            speaker_name=name[:7]
            speaker_dir_name=os.path.join(root_path+'wav',speaker_name)
            if os.path.isdir(speaker_dir_name):
                if os.path.exists(os.path.join(speaker_dir_name,name+'.wav')):
                    wav_file=os.path.join(speaker_dir_name,name+'.wav')
                    text = re.sub("[%$]+", "", text)
                    items.append({"text":text,"audio_file":wav_file,"speaker_name":speaker_name})
...
def custom_caiyinwen():
    pass
# def MagicDATA(root_path:str,meta_file:str,**kwargs) ->List[list[str]]:
#
#     items=[]
#     import re
#     with open(meta_file,'r',encoding='utf-8') as f:
#         all_lables=f.readlines()
#         for per_line in all_lables:
#             print(per_line)
#             exit()
#
#         pass

def magicdata(root_path:str,meta_file:str,**kwargs):

    items=[]
    import re
    with open(os.path.join(root_path,meta_file),'r',encoding='utf-8') as f:
        all_lables=f.readlines()
        for per_line in tqdm(all_lables[1:]):
            uttence_id,speaker_id,text=per_line.strip().split('\t')
            speaker_dir_name=os.path.join(root_path,speaker_id)
            if os.path.isdir(speaker_dir_name):
                if os.path.exists(os.path.join(speaker_dir_name,uttence_id)):
                    wav_file=os.path.join(speaker_dir_name,uttence_id)
                    items.append({"text": text, "audio_file": wav_file, "speaker_name": speaker_id})
...
#     root_path='/data/zll/yanghan/data/audio/MAGICDATA/train'
#     meta_file='/data/zll/yanghan/data/audio/MAGICDATA/train/TRANS.txt'
#     all_items=MagicDATA(root_path=root_path,meta_file=meta_file)
#     print(len(all_items))
#     pass
